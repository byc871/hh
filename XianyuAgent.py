...
class XianyuReplyBot:
    ...
    def _init_system_prompts(self):
        ...
        try:
            # 加载分类提示词
            with open(os.path.join("prompts", "classify_prompt.txt"), "r", encoding="utf-8") as f:
            ...
            # 加载图片提示词
            with open(os.path.join("prompts", "image_prompt.txt"), "r", encoding="utf-8") as f:
                self.image_prompt = f.read()
            logger.debug(f"已加载图片提示词，长度: {len(self.image_prompt)} 字符")
            
            logger.info("成功加载所有提示词")
            return True
        except Exception as e:
            logger.error(f"初始化系统提示词失败: {e}")
            return False
